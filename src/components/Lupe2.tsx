...
import { AlertTriangle, Send, Mic, MicOff } from 'lucide-react';
import { Loader2, MessageSquare, Volume2, ArrowDown } from 'lucide-react';

interface Lupe2Props {
  language?: 'en' | 'es' | 'zh' | 'hi' | 'ar';
}

const Lupe2: React.FC<Lupe2Props> = ({ language = 'en' }) => {
  ...
  // Try to auto-start the conversation when component mounts, but only once
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4 overflow-hidden">
      <div className="max-w-2xl w-full flex flex-col h-[90vh] gap-4">
        {/* Introduction text */}
        ...
        <div className="bg-black/40 backdrop-blur-sm rounded-lg overflow-hidden shadow-lg border border-gray-700 flex-1 flex flex-col">
          ...
          <div 
            ...
            className="overflow-y-auto p-4 space-y-4 h-64 bg-gray-900/50"
          ...
          {showScrollButton && (
            <button
              ...
              className="absolute bottom-[180px] right-4 p-2 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-colors"
            >
              <ArrowDown size={20} />
            </button>
          )}
