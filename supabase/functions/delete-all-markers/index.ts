...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { limit, dryRun } = await req.json();
    ...
    if (isDryRun) {
      return new Response(
        JSON.stringify({
          message: `Dry run - would delete ${markersToDelete.length} markers`,
    ...
    if (markersToDelete.length > 0) {
      ...
      return new Response(
        JSON.stringify({
          message: `Successfully deleted ${markersToDelete.length} markers`,
      ...
      return new Response(
        JSON.stringify({
          message: `No markers found to delete`,
          deletedMarkers: [],
          dryRun: false
        }),
        {
