...
BEGIN
  ...
  -- A marker is inactive if:
  -- 1. It's over 24 hours old AND
  -- 2. It's not a newly created marker (within last 5 minutes)
  IF NEW.last_confirmed < NOW() - INTERVAL '24 hours' AND 
...
-- This is a one-time update that doesn't rely on functions
UPDATE markers
SET 
  active = CASE 
...
-- Create a separate function for manual cleanup that can be called
-- but won't be triggered automatically to avoid recursion
CREATE OR REPLACE FUNCTION manual_cleanup_inactive_markers()
RETURNS void AS $$
BEGIN
  UPDATE markers
  SET 
